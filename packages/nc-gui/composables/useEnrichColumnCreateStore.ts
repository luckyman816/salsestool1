...
const [useProvideEnrichColumnCreateStore, useEnrichColumnCreateStore] = createInjectionState(
  (
    ...
    const addOrUpdateEnrich = async (onSuccess: () => Promise<void>, columnPosition?: Pick<ColumnReqType, 'column_order'>) => {
      try {
...
export { useProvideEnrichColumnCreateStore }

export function useEnrichColumnCreateStoreOrThrow() {
  const enrichColumnCreateStore = useEnrichColumnCreateStore()

  if (enrichColumnCreateStore != null) throw new Error('Please call `useProvideColumnCreateStore` on the appropriate parent component')

  return enrichColumnCreateStore
}
