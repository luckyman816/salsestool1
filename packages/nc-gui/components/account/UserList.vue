...
const loadUsers = useDebounceFn(async (page = currentPage.value, limit = currentLimit.value) => {
  ...
  try {
    ...
    usersTemp.value = response.list as UserType[]

    usersTemp.value.forEach(function (user) {
        
      if(user.team_email === loggedInUser.value?.email ) users.value.push(user)
