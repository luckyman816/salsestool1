...
import { useWorkspace, useDebounceFn, useApi } from '#imports'
import type { NcProject } from '#imports'
import { OrgUserRoles } from 'nocodb-sdk'
import type { OrgUserReqType, UserType } from 'nocodb-sdk'
...
export const useBases = defineStore('basesStore', () => {
  ...
  const loadProjects = async (page: 'recent' | 'shared' | 'starred' | 'workspace' = 'recent') => {
    ...
    const workspace = workspaceStore.workspaces

    if ((!page || page === 'workspace') && !workspace?.id && !activeWorkspace?.id) {
      throw new Error('Workspace not selected')
    }

    let _projectsTemp: BaseType[] = []
    ...
    try {
      ...
      _projectsTemp = list
      if(user.value?.roles.super === true ){
         _projects = _projectsTemp.filter((_project) => _project.team_email === user.value?.email);
      } else {
         _projects = _projectsTemp.filter((_project) => (_project.user_id === user.value?.id));
      }
