...
export class BasesService {
  ...
  async baseList(param: {
    ...
    //console.log("Current user's role is : "+ extractRolesObj(param.user?.roles)[OrgUserRoles.SUPER_ADMIN])
  ...
  async baseCreate(param: { base: ProjectReqType; user: any; req: NcRequest }) {
    ...
    const loggeduser = await User.getByEmail(baseBody.email);
  
    baseBody.team_email = loggeduser.team_email;
  ...
  async createDefaultBase(param: { user: UserType; req: NcRequest }) {
    console.log("create project user id", param.user.id)
